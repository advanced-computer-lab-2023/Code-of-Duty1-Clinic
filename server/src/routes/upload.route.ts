...
import {
    saveMedicalHistory, removeMedicalHistory, saveRegistrationFiles, getMedicalHistoryURL,
    getMedicalHistory, getAllRequests, getRequestFileUrl
} from '../services'
...
uploadRouter.get('/medicine/image/:id', async (req, res) => {
    ...
    if (url)
        ...
        res.status(StatusCodes.NOT_FOUND).send("URL not found check did you upload any picture ?");
});
uploadRouter.use(isAuthenticated);
...
uploadRouter.post("/pharmacist/registration", registrationUpload.fields(allowedRegistrationFields as any[]),(req, res) => {
    controller(res)(saveRegistrationFiles)(req.decoded.id, req.files);
});
