    ...
    describe('Testing Login', () => {
        describe("Login as invalid user", () => {
            ...
            it("Login with valid username and invalid password", async () => {
                await expect( login({ username: users["Patient"][0].username, password: "47851" })).rejects.toThrow('Incorrect password');
                await expect( login({ username: users["Patient"][0].username, password: "" })).rejects.toThrow('Username and password are required');
            });
        });
